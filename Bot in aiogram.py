    ...
    if WordlyBot.word == "":
        ...
        for i in message_text:
            if i in s and WordlyBot.word.index(i) == message_text.index(i):
                ...
                if (
                    i not in WordlyBot.letters_not_in_word
                    and i not in WordlyBot.letters_in_word
                ):
        ...
        if WordlyBot.b < 5:
            ...
            await message.answer(
                "Буквы не в слове: "
                + str(sorted(WordlyBot.letters_not_in_word))[1:-1].replace(
                    "'", ""
                )
            )
            await message.answer(
        ...
        if WordlyBot.b == 6:
            await message.answer(
                "Вы проиграли! Загаданное слово было: " + WordlyBot.word
            )
            await message.answer(
