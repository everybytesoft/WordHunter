"""Этот скрипт преобразует данные в формате двумерного массива в векторное изображение в формате SVG, и затем сохраняет его как PNG."""

import cairosvg
from typing import List, Union


def svg_get_opacity(text: str) -> str:
    """Функция определяет прозрачность элемента в зависимости от текста"""
    return ' opacity="0.3"' if text == " " else ""


def svg_square(color: int, text: str, x: int, y: int) -> str:
    """Функция создает SVG-элемент квадрата с текстом внутри"""
    return f'<rect fill="url(#{color})" x="{x * 32 + 2}" y="{y * 32 + 2}"{svg_get_opacity(text)} rx="5" width="30" height="30"/><text fill="#fff" font-size="24" font-family="Arial Unicode MS" x="{x * 32 + 1 + (32-24) * 0.6 * 2}" y="{y * 32 + 24 + 1}">{text}</text>'


def svg_grid(data: List[Union[str, int]]) -> None:
    """Функция генерирует весь SVG-код на основе данных"""
    str: str = ""
    for i in range(6):
        for j in range(5):
            str += svg_square(int(data[i][j][1]), data[i][j][0], j, i)

    svg: str = (
        f'<svg xmlns="http://www.w3.org/2000/svg" width="162" height="194"><defs><linearGradient id="0" gradientTransform="rotate(90)"><stop stop-color="#4f4f4f" offset="0"/><stop stop-color="#212121" offset="1"/></linearGradient><linearGradient id="1" gradientTransform="rotate(90)"><stop stop-color="#ffec8e" offset="0"/><stop stop-color="#d1ab00" offset="1"/></linearGradient><linearGradient id="2" gradientTransform="rotate(90)"><stop stop-color="#8eff97" offset="0"/><stop stop-color="#00d10e" offset="1"/></linearGradient></defs><rect fill="#000" width="162" height="194"/>{str}</svg>'
    )

    with open("vector.svg", "w") as f:
        f.write(svg)

    with open("vector.svg", "r") as f:
        svg_data = f.read()

    cairosvg.svg2png(bytestring=svg, write_to="output.png")
