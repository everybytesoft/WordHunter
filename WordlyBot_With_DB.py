...
WHERE id={user_id};"""
    ...
    def process_text_message(self, message: telebot.types.Message) -> None:
        ...
        if word == "":
            self.send_message(
                message.from_user.id, "Игра еще не началась! Напишите /play"
            )
        elif b == 6:
            self.send_message(
                message.from_user.id,
            ...
            self.send_message(
                message.from_user.id, "Введите существующее слово из 5 букв!"
            )
        elif message_text in list_of_used_words:
            self.send_message(
                message.from_user.id,
                "Вы уже писали это слово, выберите другое",
            )
            ...
            for i in message_text:
                if i in s and word.index(i) == message_text.index(i):
                    ...
                    if (
                        i not in letters_not_in_word
                        and i not in letters_in_word
                    ):
            ...
            if b < 5:
                self.send_message(
                    message.from_user.id,
                    "Буквы в слове: "
                    + str(sorted(letters_in_word))[1:-1].replace("'", ""),
                )
                self.send_message(
                    message.from_user.id,
                    "Буквы не в слове: "
                    + str(sorted(letters_not_in_word))[1:-1].replace("'", ""),
                )
                self.send_message(
                    message.from_user.id,
                    "Неиспользованные буквы: "
                    + str(letters_is_not_used)[1:-1].replace("'", ""),
                )
            b += 1
            if b == 6:
                self.send_message(
